      ...
      LVal: function LVal (path, file) {
        ...
        if (watch_variables.hasOwnProperty(name)) {
          ...
          if (path.parent.type === "VariableDeclarator") {
            // This is has a bug now, since we need to check that this is the first one
        ...
        exit: function ProgramExit (path, file) {
          ...
          each(watch_variables, function (val, key) {
            if (val.seen && !val.declared) {
              console.log("Creating shim for " + key);
              console.log(path_module.basename(file.file.opts.filename));
